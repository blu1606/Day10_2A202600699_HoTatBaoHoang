...
from transform.cleaning_rules import clean_rows, load_raw_csv, load_source_docs, write_cleaned_csv, write_quarantine_csv
...
def cmd_run(args: argparse.Namespace) -> int:
    ...
    raw_export_count = len(rows)
    source_doc_rows = []
    if not args.raw_only:
        source_doc_rows = load_source_docs(ROOT / "data" / "docs", exported_at=run_timestamp)
        rows.extend(source_doc_rows)
    raw_count = len(rows)
    log(f"run_id={run_id}")
    log(f"raw_export_records={raw_export_count}")
    log(f"source_doc_records={len(source_doc_rows)}")
    ...
    manifest = {
        ...
        "raw_export_records": raw_export_count,
        "source_docs_enabled": not args.raw_only,
        "source_doc_records": len(source_doc_rows),
...
def main() -> int:
    ...
    p_run.add_argument(
        "--raw-only",
        action="store_true",
        help="Tắt ingest data/docs/*.txt mặc định, chỉ chạy raw CSV để so sánh.",
    )
